Create one horizontal animation strip for Codex pet `kitten`, state `running`.
...
Behavior note: active investigation with alert suspicion, not literal sprinting. She can look like she is deciding whether to bite the cursor, but do not make every frame a full attack.
