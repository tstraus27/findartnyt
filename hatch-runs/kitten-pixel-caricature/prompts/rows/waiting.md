Create one horizontal animation strip for Codex pet `kitten`, state `waiting`.
...
Behavior note: expectant but slightly suspicious; she is peaceful unless provoked, so use alert ears and visible underbite without full attack.
